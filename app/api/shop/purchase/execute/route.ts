...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (item[0].category === 'NFT') {
      ...
      try {
        ...
        const IPFS_MAPPING: Record<string, string> = {
          ...
          nft_aurum_white_tiger:
            'ipfs://bafybeic7y5qkv34fclloygbntzf63tbdzvhlarrdnt7sn6opvucmlc3pze',
          nft_aurum_black_turtle:
            'ipfs://bafybeicddiccbudqjtblgkfs4fgm7so3vciljx26rmaioc3nr5dlvwwkwu',
    ...
    const errorCode = message.includes('NONCE')
      ? 'NONCE_ERROR'
      : message.includes('MISMATCH')
        ? 'VALIDATION_ERROR'
        : 'EXECUTE_FAILED';

    return Response.json({ success: false, error: errorCode, message }, { status: 500 });
  }
}
