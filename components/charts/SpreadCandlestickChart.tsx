...
export default function SpreadCandlestickChart({
    ...
    const fetchData = async () => {
        try {
            ...
            const candleData: CandleData[] = slicedData.map((item: any) => ({
        ...
        <div className="w-full">
            ...
            <div className="mb-4 grid grid-cols-3 gap-4">
                <div className="rounded-lg bg-gray-800/50 p-3">
                    ...
                    <div className={`text-2xl font-bold ${stats.currentWinner === 'PAXG' ? 'text-red-400' : stats.currentWinner === 'BTC' ? 'text-blue-400' : 'text-gray-400'}`}>
                        {stats.currentWinner === 'TIE' ? '동률' : stats.currentWinner}
                    </div>
                    <div className="text-xs text-gray-500">
                        {stats.currentWinner === 'PAXG' ? '🔴 양봉' : stats.currentWinner === 'BTC' ? '🔵 음봉' : '➡️'}
                    </div>
                </div>

                <div className="rounded-lg bg-gray-800/50 p-3">
                    <div className="text-sm text-gray-400">격차</div>
                    <div className={`text-2xl font-bold ${Math.abs(stats.currentSpread) >= 0.5 ? 'text-yellow-400' : 'text-gray-300'}`}>
                ...
                <div className="rounded-lg bg-gray-800/50 p-3">
                    ...
                    <div className={`text-2xl font-bold ${stats.paxgWinRate >= 50 ? 'text-red-400' : 'text-blue-400'}`}>
                ...
                <ComposedChart data={data} margin={{ top: 5, right: 30, left: 20, bottom: 5 }}>
                        ...
                        formatter={(value: number, name: string, props: any) => {
                            ...
                            return [
                                <div key="tooltip" className="space-y-1">
                                    <div className={`font-bold ${payload.winner === 'PAXG' ? 'text-red-400' : 'text-blue-400'}`}>
                                        {payload.winner} 우세
                                    </div>
                    ...
                    <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                        {data.map((entry, index) => (
                            <Cell
                                key={`cell-${index}`}
                                fill={entry.winner === 'PAXG' ? '#f87171' : '#60a5fa'}
                            />
                        ))}
                    </Bar>
                </ComposedChart>
