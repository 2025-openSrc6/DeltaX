...
export function BetCard({ roundType = 'DEMO_3MIN' }: BetCardProps) {
    ...
    const [myBets, setMyBets] = useState<Array<{ id: string; prediction: string; amount: number; resultStatus: string | null; claimed: boolean }>>([]);;
    ...
    // 승리 베팅 (클레임 가능) - backend uses 'WON', 'LOST', 'REFUNDED'
    const winningBets = myBets.filter(
        bet => bet.resultStatus === 'WON' && !bet.claimed
    );
    ...
    const handleClaim = async (betId: string) => {
        ...
        if (result.success) {
            setClaimResult({ success: true, message: `클레임 성공! 판아액: ${result.payoutAmount} DEL` });
            // 베팅 목록 새로고침
    ...
    const currentGoldPrice = goldData30 && goldData30.length > 0 ? goldData30[goldData30.length - 1].close : 0;
    const currentBtcPrice = btcData30 && btcData30.length > 0 ? btcData30[btcData30.length - 1].close : 0;
            ...
            {isSettled && (
                <div className="p-6 bg-gradient-to-b from-gray-900 to-gray-800 rounded-b-lg">
                    <div className="text-center mb-6">
                        ...
                        <h3 className="text-2xl font-bold text-white mb-2">라운드 #{round?.roundNumber} 종료!</h3>
                        {round?.winner ? (
                            <div className={`text-3xl font-extrabold ${round.winner === 'GOLD' ? 'text-yellow-400' : 'text-orange-400'}`}>
                            ...
                            {winningBets.length > 0 ? (
                                <div className="bg-green-900/40 border border-green-500/50 rounded-lg p-4">
                                    ...
                                    <div className="space-y-2">
                                        {winningBets.map(bet => (
                                            <div key={bet.id} className="flex items-center justify-between bg-gray-800/50 rounded-lg p-3">
                                    ...
                                    {claimResult && (
                                        <div className={`mt-3 p-2 rounded text-center text-sm ${claimResult.success ? 'bg-green-800/50 text-green-300' : 'bg-red-800/50 text-red-300'}`}>
                                            {claimResult.message}
                                        </div>
                                    )}
                                </div>
            ...
            {canBet && (
                                ...
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <div className="text-right">
                                        <p className="text-xl font-bold text-stone-800">
                                            ${currentGoldPrice.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </p>
                                        <p className={`text-xs ${goldChangePercent >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                ...
                `}
                            >
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <p className={`text-xs ${btcChangePercent >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    ...
                    <div className="px-6 pb-6 space-y-4">
                        ...
                        {betResult && (
                            <div
                                className={`px-4 py-3 rounded-lg text-sm ${betResult.success
